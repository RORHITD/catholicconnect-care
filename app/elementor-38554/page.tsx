    ...
    <section className="py-16 lg:py-24">
      <div className="mx-auto max-w-4xl px-6 lg:px-8">
        <header className="text-center">
          ...
          <h1 className="mt-4 font-display text-4xl text-burgundy-900 md:text-5xl">
            Daily Readings of the Catholic Church
          </h1>
          <p className="mx-auto mt-5 max-w-2xl text-stone-warm-700">
        ...
        <ul className="mt-12 space-y-4">
          {items.map((item) => (
            <li key={item.link} className="rounded-2xl bg-cream-50 p-6 ring-1 ring-stone-warm-200">
              <h2 className="font-display text-xl text-burgundy-800">
                <Link href={item.link} target="_blank" rel="noopener noreferrer" className="hover:text-gold-700 transition">
                  {decodeHtml(item.title)}
                </Link>
              </h2>
              {item.pubDate && (
                <time className="mt-1 block text-xs text-stone-warm-600">
                  {new Date(item.pubDate).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })}
                </time>
              )}
              {item.description && (
                <p className="mt-3 text-sm leading-relaxed text-stone-warm-700">
              ...
              <Link
                ...
                className="mt-4 inline-flex items-center gap-1.5 text-sm font-semibold text-burgundy-700 hover:text-gold-600 transition"
              >
                Read at USCCB
